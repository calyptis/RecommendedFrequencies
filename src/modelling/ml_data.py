...
from src.spotify.config import (MAIN_DATA_FILE, PLAYLIST_FILE, SIMILAR_PLAYLISTS_FILE)
from src.spotify.utils import read_pickle
from src.modelling.config import (EUCLIDEAN_FEAT_COLS, CATBOOST_FEATURES, EVERYNOISE_FEAT_COL)


def create_song_triplets() -> pd.DataFrame:
    """
    Creates dataframe holding triplets of songs in addition to the playlist context.
    ...
    pos_examples = np.array([(i[0], i[1], i[2]["playlist"]) for i in graph.edges(data=True)])
    # Add the same examples with the pairs switched
    switched_pos_examples = np.array([(i[1], i[0], i[2]["playlist"]) for i in graph.edges(data=True)])
    # Add some more positive examples => both inputs are the same song
    frac = 0.4
    tmp = np.random.choice(list(graph.nodes()), size=int(pos_examples.shape[0] * frac)).flatten()
    # TODO: Pick a playlist the song appears instead of randomly choosing one
    addtl_pos_examples = np.array([(i, i, np.random.choice(list(playlists.keys()))) for i in tmp])
    pos_examples = np.concatenate((pos_examples, switched_pos_examples, addtl_pos_examples))
    ...
    df_playlists_similarity["different_playlists"] = (
        df_playlists_similarity
        .similar_playlists
        .apply(lambda x: list(set(playlists.keys()) - x))
    )
    ...
    for i in range(pos_examples.shape[0]):
        ...
        negative_example_song = np.random.choice(playlists[different_playlist]["tracks"])
...
def create_song_pair_features(df_triplets_examples: pd.DataFrame):
    ...
    df_pairs_examples = pd.concat((
        ...
        [["anchor", "positive_example", "playlist"]]
        .rename(columns={"positive_example": "example"})
        .assign(target=1),
        df_triplets_examples
        [["anchor", "negative_example", "playlist"]]
...
def create_graph(df: pd.DataFrame, playlists: dict) -> nx.classes.graph.Graph:
    ...
    graph = nx.from_pandas_edgelist(edges, source="song_1", target="song_2", edge_attr="playlist")
    print(f"Number of nodes: {len(graph.nodes):,}")
    print(f"Number of edges: {len(graph.edges):,}")

    # Node attribute is the list of playlist the song featured in
    node_attributes = pd.concat((
        edges[["song_1", "playlist"]].rename(columns={"song_1": "song"}),
        edges[["song_2", "playlist"]].rename(columns={"song_2": "song"})
    )).groupby("song").playlist.agg(lambda x: "; ".join(set(x))).reset_index()
    nx.set_node_attributes(graph, values=node_attributes.set_index("song").to_dict(orient="index"))
