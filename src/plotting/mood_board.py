...
def plot_mood_board(playlist_features, title, inline=False, metrics_version=1):
    ...
    if metrics_version == 1:
        ...
        upper_bound = main_line + playlist_features_summary.loc["std", RADIAL_COLS].values
        lower_bound = main_line - playlist_features_summary.loc["std", RADIAL_COLS].values
    elif metrics_version == 2:
        # Median and bottom / top 10th percentile as variation
        main_line = np.array([np.percentile(playlist_features[c], 50) for c in RADIAL_COLS])
        upper_bound = np.array([np.percentile(playlist_features[c], 90) for c in RADIAL_COLS])
        lower_bound = np.array([np.percentile(playlist_features[c], 10) for c in RADIAL_COLS])
    else:
