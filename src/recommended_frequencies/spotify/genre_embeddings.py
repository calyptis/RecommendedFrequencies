...
def download_everynoise_genre_space() -> None:
    ...
    df_genres_positions.to_json(
        SpotifyFiles.GENRE_COORDINATES, lines=True, orient="records"
    )
...
def get_everynoise_embeddings() -> None:
    ...
    genres_df = pd.read_json(SpotifyFiles.GENRE_FILE, lines=True, orient="records")
    genre_space_df = pd.read_json(
        SpotifyFiles.GENRE_COORDINATES, orient="records", lines=True
    ).set_index("genre")
    ...
    genres_df.to_json(SpotifyFiles.GENRE_EMBEDDING_SPACE, lines=True, orient="records")
